...
function ActivityEditor({
  ...
  return <fieldset className="owner-activity">
    ...
    {activity.type === "external_media" && activity.content.descriptor && <>
      <div className="form-row">
        <label className="field"><span>Media provider</span><select value={activity.content.descriptor.provider} onChange={(event) => { const provider = event.target.value as NonNullable<WorkflowActivity["content"]["descriptor"]>["provider"]; const instrumented = provider === "youtube" || provider === "vimeo"; update({ ...activity, content: { ...activity.content, completionPolicy: { ...(activity.content.completionPolicy || { minimumUniqueSeconds: 1, mode: "acknowledgement", thresholdPercent: 90 }), mode: instrumented ? "playback_or_acknowledgement" : "acknowledgement" }, descriptor: { ...activity.content.descriptor!, provider } } }); }}><option value="youtube">YouTube</option><option value="vimeo">Vimeo</option><option value="sharepoint">SharePoint / OneDrive</option><option value="google_drive">Google Drive</option><option value="dropbox">Dropbox</option><option value="generic">Approved generic embed</option><option value="external_link">External link</option></select></label>
        <label className="field"><span>Media kind</span><select value={activity.content.descriptor.kind} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, kind: event.target.value as NonNullable<WorkflowActivity["content"]["descriptor"]>["kind"] } } })}><option value="video">Video</option><option value="audio">Audio</option><option value="image">Image</option><option value="presentation">Presentation</option><option value="document">Provider document</option></select></label>
      </div>
      <label className="field"><span>Canonical provider URL</span><input type="url" value={activity.content.descriptor.sourceUrl} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, sourceUrl: event.target.value } } })} placeholder="https://…" required /></label>
      {(["sharepoint", "generic"].includes(activity.content.descriptor.provider)) && <label className="field"><span>Provider embed or preview URL</span><input type="url" value={activity.content.descriptor.embedUrl || ""} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, embedUrl: event.target.value } } })} placeholder="https://…" required /></label>}
      <div className="form-row">
        <label className="field"><span>Participant-facing media title</span><input value={activity.content.descriptor.title} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, title: event.target.value } } })} minLength={2} maxLength={200} required /></label>
        <label className="field"><span>Stable provider item ID</span><input value={activity.content.descriptor.itemId || ""} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, itemId: event.target.value } } })} placeholder="Extracted automatically where supported" required={activity.content.descriptor.provider === "sharepoint"} /></label>
      </div>
      <label className="field"><span>Description (optional)</span><textarea value={activity.content.descriptor.description || ""} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, description: event.target.value } } })} maxLength={2000} /></label>
      <div className="form-row">
        <label className="field"><span>Access expectation</span><select value={activity.content.descriptor.accessMode} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, accessMode: event.target.value as NonNullable<WorkflowActivity["content"]["descriptor"]>["accessMode"] } } })}><option value="public">Public</option><option value="provider_shared">Shared by provider link</option><option value="provider_authenticated">Provider account required</option></select></label>
        <label className="field"><span>Known duration in seconds</span><input type="number" min="0.25" max="604800" step="0.25" value={activity.content.descriptor.durationSeconds ?? ""} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, durationSeconds: event.target.value ? Number(event.target.value) : undefined } } })} /></label>
      </div>
      <div className="form-row">
        <label className="field"><span>Provider version / revision</span><input value={activity.content.descriptor.version?.id || ""} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, version: { ...activity.content.descriptor!.version, id: event.target.value } } } })} /></label>
        <label className="field"><span>Provider eTag / change token</span><input value={activity.content.descriptor.version?.eTag || ""} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, version: { ...activity.content.descriptor!.version, eTag: event.target.value } } } })} /></label>
      </div>
      <label className="field"><span>Provider checksum (if available)</span><input value={activity.content.descriptor.version?.checksum || ""} onChange={(event) => update({ ...activity, content: { ...activity.content, descriptor: { ...activity.content.descriptor!, version: { ...activity.content.descriptor!.version, checksum: event.target.value } } } })} /></label>
      <label className="field"><span>Participant prompt</span><input value={activity.content.prompt || ""} onChange={(event) => update({ ...activity, content: { ...activity.content, prompt: event.target.value } })} required /></label>
      <div className="form-row">
        <label className="field"><span>Completion policy</span><select value={activity.content.completionPolicy?.mode || "acknowledgement"} onChange={(event) => update({ ...activity, content: { ...activity.content, completionPolicy: { ...(activity.content.completionPolicy || { minimumUniqueSeconds: 1, thresholdPercent: 90 }), mode: event.target.value as NonNullable<WorkflowActivity["content"]["completionPolicy"]>["mode"] } } })}><option value="acknowledgement">Acknowledgement</option>{(["youtube", "vimeo"].includes(activity.content.descriptor.provider)) && <><option value="playback">Validated playback required</option><option value="playback_or_acknowledgement">Playback or acknowledgement</option></>}</select></label>
        <label className="field"><span>Playback threshold (%)</span><input type="number" min="1" max="100" value={activity.content.completionPolicy?.thresholdPercent ?? 90} onChange={(event) => update({ ...activity, content: { ...activity.content, completionPolicy: { ...(activity.content.completionPolicy || { minimumUniqueSeconds: 1, mode: "acknowledgement" }), thresholdPercent: Number(event.target.value) } } })} /></label>
      </div>
      <div className="form-row">
        <label className="field"><span>Minimum unique playback seconds</span><input type="number" min="0" max="604800" value={activity.content.completionPolicy?.minimumUniqueSeconds ?? 1} onChange={(event) => update({ ...activity, content: { ...activity.content, completionPolicy: { ...(activity.content.completionPolicy || { mode: "acknowledgement", thresholdPercent: 90 }), minimumUniqueSeconds: Number(event.target.value) } } })} /></label>
        <label className="field"><span>Telemetry heartbeat (seconds)</span><input type="number" min="2" max="60" value={activity.content.telemetryPolicy?.heartbeatSeconds ?? 5} onChange={(event) => update({ ...activity, content: { ...activity.content, telemetryPolicy: { ...(activity.content.telemetryPolicy || { idleSeconds: 30, maxCreditedGapSeconds: 10 }), heartbeatSeconds: Number(event.target.value) } } })} /></label>
      </div>
      <div className="form-row">
        <label className="field"><span>Idle limit (seconds)</span><input type="number" min={activity.content.telemetryPolicy?.heartbeatSeconds ?? 5} max="900" value={activity.content.telemetryPolicy?.idleSeconds ?? 30} onChange={(event) => update({ ...activity, content: { ...activity.content, telemetryPolicy: { ...(activity.content.telemetryPolicy || { heartbeatSeconds: 5, maxCreditedGapSeconds: 10 }), idleSeconds: Number(event.target.value) } } })} /></label>
        <label className="field"><span>Maximum credited telemetry gap</span><input type="number" min={activity.content.telemetryPolicy?.heartbeatSeconds ?? 5} max="120" value={activity.content.telemetryPolicy?.maxCreditedGapSeconds ?? 10} onChange={(event) => update({ ...activity, content: { ...activity.content, telemetryPolicy: { ...(activity.content.telemetryPolicy || { heartbeatSeconds: 5, idleSeconds: 30 }), maxCreditedGapSeconds: Number(event.target.value) } } })} /></label>
      </div>
      <label className="field"><span>Acknowledgement wording</span><input value={activity.content.acknowledgementLabel || ""} onChange={(event) => update({ ...activity, content: { ...activity.content, acknowledgementLabel: event.target.value } })} required /></label>
      <label className="field"><span>Accessibility alternative label</span><input value={activity.content.accessibilityAlternative?.label || ""} onChange={(event) => update({ ...activity, content: { ...activity.content, accessibilityAlternative: { ...(activity.content.accessibilityAlternative || {}), label: event.target.value } } })} /></label>
      <label className="field"><span>Accessibility alternative URL (optional)</span><input type="url" value={activity.content.accessibilityAlternative?.url || ""} onChange={(event) => update({ ...activity, content: { ...activity.content, accessibilityAlternative: activity.content.accessibilityAlternative ? { ...activity.content.accessibilityAlternative, url: event.target.value } : undefined } })} /></label>
      <p className="participant-limit">VASI stores an immutable descriptor and available version reference. Provider media bytes remain external, and capabilities downgrade honestly when the provider does not expose playback events.</p>
    </>}
...
function activityForType(type: WorkflowActivity["type"], index: number, artifacts: OwnerArtifact[]): EditableActivity {
  ...
  switch (type) {
    ...
    case "external_media": return { ...base, content: { acknowledgementLabel: "I confirm that I reviewed the available media or accessibility alternative.", accessibilityAlternative: { label: "Open an accessible alternative" }, completionPolicy: { minimumUniqueSeconds: 1, mode: "playback_or_acknowledgement", thresholdPercent: 90 }, descriptor: { accessMode: "public", kind: "video", provider: "youtube", sourceUrl: "", title: "Hosted media" }, prompt: "Review the hosted media and complete this activity.", telemetryPolicy: { heartbeatSeconds: 5, idleSeconds: 30, maxCreditedGapSeconds: 10 } }, responseMode: "external_media", type };
