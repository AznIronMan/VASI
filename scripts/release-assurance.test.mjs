...
describe("release assurance policy", () => {
  ...
  it("requires an explicit non-root readability contract for every release image role", () => {
    expect(runtimeContractForImage("vasi:0.40.1")).toMatchObject({
    ...
    expect(runtimeContractForImage("registry.example.test/vasi-engine:0.40.1")).toMatchObject({
      entrypoints: [
    ...
    expect(runtimeContractForImage("vasi-engine-maintenance:0.40.1")).toMatchObject({
      entrypoints: [
    ...
    expect(runtimeContractForImage("vasi-database-gateway:0.40.1")).toMatchObject({
  ...
  it("derives a bounded physical prohibition inventory from the exact lock graph", async () => {
    ...
    const allowed = runtimeContractForImage("vasi:0.40.1").allowedOptionalPackagePaths;
