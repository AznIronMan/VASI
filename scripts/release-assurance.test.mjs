...
describe("release assurance policy", () => {
  ...
  it("requires an explicit non-root readability contract for every release image role", () => {
    expect(runtimeContractForImage("vasi:0.21.3")).toMatchObject({
      entrypoints: ["server.js"],
      imageUser: "node",
      runUser: "1000:1000",
    });
    expect(runtimeContractForImage("registry.example.test/vasi-engine:0.21.3")).toMatchObject({
      entrypoints: [
    ...
    expect(runtimeContractForImage("vasi-database-gateway:0.21.3")).toMatchObject({
